...
def place_axes_on_grid(
    ...
    """
    Takes a figure with a gridspec defined and places an array of sub-axes on a portion
    of the gridspec.

    Takes as arguments:
        fig: figure handle - required
        dim: number of rows and columns in the subaxes - defaults to 1x1
        xspan: fraction of figure that the subaxes subtends in the x-direction
              (0 = left edge, 1 = right edge)
        yspan: fraction of figure that the subaxes subtends in the y-direction
              (0 = top edge, 1 = bottom edge)
        wspace and hspace: white space between subaxes in vertical and horizontal
    ...
    inner_ax = dim[0] * [
        dim[1] * [fig]
    ]  # fill with figure objects to prevent later errors
...
def add_labels(fig, labels):
    ...
    inputs:
        fig: figure handle
        labels: a list of dictionaries with the following key/value pairs:
            * label_text (required): text of label
            * xpos (required): float from 0 to 1 defining horizontal position
            * ypos (required): float from 0 to 1 defining vertical position
            * any additional keyword arguments that can be passed to the matplotlib
              text function (e.g., fontsize, weight, etc)
    """
    for label in labels:
        add_label(fig, **label)
